...
- **Depends on:** MF-070 (Muon/local-window/NoPE pre-work methodology)
- **Deliverable:** a real, matched-token bounded comparison (reusing the `compare_*.py`/`data/shards/mf064-150m-train` methodology already established this session) measuring whether z-loss (PaLM-style `λ·log²(Σexp z)` logit-magnitude penalty), LayerNorm scaling ("The Curse of Depth in LLMs", arXiv:2502.05795, `1/√l` per-layer downweighting), normalized value residuals (mixing `V^(1)` into every layer's `V^(l)` via a learned, zero-initialized gate), and independently-tuned local-vs-global RoPE theta actually help this project's model/data/hardware, each independently ablatable and config-gated as the source suggested.
- **Acceptance:** each intervention lands behind its own `ModelConfig`/`TrainingConfig` flag, off by default; a real bounded run (same discipline as the local-window/NoPE pre-work: matched tokens, seed, batch size, real held-out validation via `evaluate_token_batches`) reports validation CE/PPL/BPB for baseline vs. each intervention vs. all-combined before any becomes part of the frozen Modern preset, per `AGENTS.md`'s "record a decision before implementation" rule.
- **Status note:** proposed by external code review, citing IMU-1 (arXiv:2602.02522) ablations on their own 70M proxy model/data/hardware (`d_model=768`, 12 heads, 4 KV heads — architecturally close to `configs/150m-modern.toml`, but not this project's own measured evidence). Their reported deltas (QK-Norm −0.63%, value residual −0.38%, LN scaling −0.36%, per-head gating −0.05%, all combined −1.64%, exceeding the individual sum) are a real, citable prior, not yet independently confirmed here — this project already learned this session (Muon-vs-AdamW, NoPE, local-window) that a paper's own numbers do not reliably transfer to this hardware/data/scale without a real bounded test. Per-head gating is intentionally split out into [[MF-082]] since it also addresses a separate, specific gap (attention-sink risk under the ring cache). Not started.
...
### MF-083 — WSD learning-rate schedule and cautious weight decay experiment

- **Priority / state:** P2 / Not started
- **Depends on:** MF-070 (AdamW-vs-Muon resolution)
- **Deliverable:** a `WarmupStableDecaySchedule` alongside the existing `WarmupCosineSchedule` (config-selectable, not a replacement), and a cautious-weight-decay AdamW variant (skip decay on a parameter when `sign(update) != sign(weight)`), each bounded-tested against the existing cosine/plain-AdamW baseline before becoming a new default.
- **Acceptance:** WSD's stable-phase length and decay-fraction/shape are explicit config fields, not hardcoded; a real matched-token comparison (reusing `compare_optimizers.py`'s methodology) shows WSD-vs-cosine and cautious-vs-plain-decay validation CE/PPL/BPB before either changes `configs/150m-modern.toml`'s defaults; WSD's actual practical benefit (branching a usable checkpoint mid-run, extending a completed run without re-choosing a schedule shape) is demonstrated with a real resumed run, not just argued from the schedule's shape.
- **Status note:** proposed by external code review alongside a "promote Muon to default" recommendation, citing IMU-1's NorMuon-over-AdamW result (−2.88%, iteration/token-matched). **That recommendation is not adopted** — this project already ran the real, decisive test this exact question needed (`reports/mf070-muon-followup.md`): at *wall-clock-matched* budgets (this project's actual constraint, not iteration count), AdamW real val PPL 90.38 beats every tested Muon configuration including a best-case hybrid (FP32 Newton-Schulz + `ns_steps=3` + Muon's best LR), real PPL 95.86. IMU-1's comparison is not wall-clock-matched and does not override this project's own already-completed, more-relevant evidence. WSD and cautious weight decay are independent of the optimizer choice and remain open, real gaps worth their own bounded test. μP (learning-rate/init transfer across this project's four size presets) was also raised — genuinely useful for the 50M/150M/350M/500M ladder, but a bigger, separate investment; noted here as a considered, deferred idea, not scheduled.

### MF-084 — Chunked cross-entropy to reduce logits memory pressure

- **Priority / state:** P2 / Not started
- **Depends on:** MF-041
- **Deliverable:** compute `lm_head` projection and cross-entropy loss over sequence-length chunks and sum, instead of materializing the full `[B, S, vocab_size]` logits (and their FP32-upcast gradient) in one piece — a real, measurable memory reduction at this project's `vocab_size=16384` on an 8GB card.
- **Acceptance:** chunked and unchunked loss agree exactly (or within documented FP32 tolerance) on the same batch; peak VRAM at a fixed batch size/context measurably drops (or, equivalently, a larger batch size/context becomes reachable at the same VRAM ceiling), measured for real on this hardware, not estimated.
- **Status note:** proposed by external code review, correctly identifying that `next_token_loss_stats`'s full `[B, S, V]` logits plus their FP32-autocast-upcast gradient is a disproportionate memory cost for a small model (cited: ~1GB+ at B=8/S=2048/V=16384, versus this model's own ~0.28GB BF16 weights). The review's other suggestion, "Cut Cross-Entropy" (arXiv:2411.09009), is **not** adopted as stated — it requires custom Triton kernels, which `AGENTS.md`'s frozen V1 scope explicitly excludes ("Do not add... custom CUDA/Triton kernels... in V1"). Chunked cross-entropy achieves the same real memory win in plain PyTorch, within scope. Not started.

### MF-085 — Document-boundary intra-document attention masking for packed shards

- **Priority / state:** P2 / Not started
- **Depends on:** MF-034 (shard packing), MF-053 (SFT packing)
...
- **Status note:** proposed by external code review. The schema addition (recording `doc_ids`) is cheap and safe to do well ahead of turning masking on — already-packed shards would need repacking to gain it, so earlier is better. The masking itself, like every other architecture change this session, needs its own bounded real test before being adopted, not just the cited paper's result. Not started.
...
- **Status note:** proposed by external code review, correctly identifying that this project's current default eval set is too thin (near-chance at 150M scale, run-to-run noise likely exceeding small effect sizes like [[MF-081]]'s ~1.6% loss delta) to actually validate any of the other proposed changes. This task is a prerequisite for evaluating [[MF-081]] and [[MF-082]] meaningfully, not just a nice-to-have — raised to P1 for that reason. Not started.

### MF-087 — Retrain the tokenizer at 32,768 vocabulary with digit-splitting

- **Priority / state:** P1 / Not started
- **Depends on:** none (frozen-scope decision already recorded, 2026-09-06, `AGENTS.md`/`docs/IMPLEMENTATION_DECISIONS.md`)
- **Deliverable:** a retrained tokenizer at `VOCAB_SIZE = 32_768` (`src/minifrontier/tokenizer.py`), with a digit-splitting pre-tokenizer step added ahead of `ByteLevel` (Llama 3/Qwen-style — splitting digit runs individually or in groups of ≤3, replacing the current GPT-2-style `ByteLevel(use_regex=True)` behavior that merges `\p{N}+` into single tokens), and every downstream artifact that hardcodes the old vocabulary updated to match.
- **Acceptance:** `train_tokenizer` produces a real tokenizer trained on real data at the new vocab size with digit-splitting verified (e.g. `"12345"` tokenizes as multiple digit-group tokens, not one merged token); all six preset TOMLs' `vocab_size = 16384` are updated to `32768`; every test asserting the old `16_384`/`VOCAB_SIZE` value is updated or parameterized; a real tokenizer-fertility comparison (bytes/token on a held-out sample) is recorded against the old tokenizer, since a larger vocabulary is only a win if fertility actually improves; existing 16,384-vocabulary checkpoints (MF-063, MF-064/065, `v0.1.0`) are explicitly documented as incompatible with the new tokenizer, not silently broken — their existing reports/records are left as historical, correct-at-the-time evidence, not corrected or deleted; any already-packed shards (`data/shards/*`) built under the old tokenizer are identified and either repacked or explicitly retired, since their token IDs are meaningless under the new vocabulary.
- **Status note:** decision recorded 2026-09-06 (user-approved after external code review, citing Tao et al., arXiv:2407.13623, that vocabulary size should scale with model size and that 16,384 under-provisions a ~138M-parameter non-vocab model). This is a real, hands-on, multi-file task — not started. `scripts/train_tokenizer.py` is the existing retraining entry point; `src/minifrontier/tokenizer.py:46` (`VOCAB_SIZE`) and `:209` (`ByteLevel(add_prefix_space=False, use_regex=True)`, the pre-tokenizer pipeline the `Split` digit step needs to precede) are the two concrete edit points identified so far. Deliberately scoped as its own task rather than folded into the MF-080 fix-pass, since it is a frozen-artifact-breaking migration, not a bug fix, and deserves its own dependency-checked, one-item-at-a-time execution per `AGENTS.md`'s task workflow.
