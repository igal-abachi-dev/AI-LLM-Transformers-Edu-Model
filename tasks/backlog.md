...
- **Status note:** implementation, CPU autocast, accumulation, and cache/argmax tests pass; CUDA BF16 parity remains the MF-063 home-RTX evidence gate.
...
- **Status note:** correctness is implemented and tested; the CUDA speed/VRAM tradeoff remains part of MF-050/MF-063.
...
- **Status note:** runtime hardening, bounded mixed local/global ring storage, wrap/chunk/reset/rollback parity, single-token local SDPA decode dispatch, and the profile/report CLI are implemented; only the home-GPU measurement matrix and recommendations remain open.
...
- **Status note:** validated draft/frozen protocol tooling is implemented. The protocol deliberately remains `draft` until the user-scheduled post-M10 RTX evidence exists; freezing without evidence hashes fails closed.
