...
- **Priority / state:** P0 / Not started — **blocks [[MF-087]]**, not merely a nice-to-have
- **Depends on:** MF-041
...
- **Status note:** proposed by external code review, correctly identifying that `next_token_loss_stats`'s full `[B, S, V]` logits plus their FP32-autocast-upcast gradient is a disproportionate memory cost for a small model. The review's other suggestion, "Cut Cross-Entropy" (arXiv:2411.09009), is **not** adopted as stated — it requires custom Triton kernels, which `AGENTS.md`'s frozen V1 scope explicitly excludes ("Do not add... custom CUDA/Triton kernels... in V1"). Chunked cross-entropy achieves the same real memory win in plain PyTorch, within scope. **Reprioritized to P0, ahead of [[MF-087]], 2026-09-06**: doubling the tokenizer's vocabulary (16,384 → 32,768, per [[MF-087]]'s now-approved decision) directly doubles this exact memory cost (verified: logits/gradient memory scales linearly with `batch_tokens × vocab_size`), on hardware already shown to hit VRAM-thrashing at modest headroom margins (`batch_size=4` caused 9.25GB thrashing at the *current* 16k vocab — MF-065's own status note). Running MF-087's larger vocabulary without this fix risks forcing a smaller batch size for every future run, which costs real throughput/wall-clock time indefinitely; doing this first makes the vocabulary change free instead. z-loss folded in per the reviewer's own follow-up ("zero wall-clock cost, well-attested, can't plausibly hurt → adopt without ablation" — see [[MF-081]]'s revised scope). Not started.
